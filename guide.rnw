...
If you're typsetting anything with math, it's usually prudent to load the following packages either in your preamble or a separate style file:
...
For example, ``\verb#$E(Y \mid X) = \beta_0 + \beta_1 X$# is a regression model'' will be typset as ``$E(Y \mid X) = \beta_0 + \beta_1 X$ is a regression model''
...
Scaling to 0.3 times the textwidth: 
...
If you aren't doing anything related to \texttt{R}, and just want to typeset text using \LaTeX\ , another neat option is to do all of your writing on \href{https://www.overleaf.com}{Overleaf}, and online platform that allows you to see your changes in real time, collaborate with others in a Google-docs style editting system, and backup your projects automatically using auto-save and version control features. 
